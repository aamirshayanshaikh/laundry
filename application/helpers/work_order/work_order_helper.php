...
function dispatch_form($new_ref="",$today="",$items=array()){
	$CI =& get_instance();
	$CI->html->sDivRow();
		$CI->html->sDivCol(10,'left',1);
			$CI->html->sBox('solid');
				$CI->html->sBoxBody(array('class'=>'paper'));
					$CI->html->sForm("work_order/dispatch_db","general-form");
						$CI->html->sDivRow();
							$CI->html->sDivCol(6);
								$params = array('class'=>'rOkay','ro-msg'=>'Reference must not be empty');
								$CI->html->inputPaper('Reference:','reference',$new_ref,null,$params);
							$CI->html->eDivCol();
							$CI->html->sDivCol(6);
								$CI->html->inputPaper('Dispatch Date:','dispatch_date',$today,null,array('class'=>'rOkay pick-date','ro-msg'=>'Dispatch date must not be empty'));
							$CI->html->eDivCol();
						$CI->html->eDivRow();
						$CI->html->sDivRow();
							$CI->html->sDivCol(6);
						...
						$CI->html->sDivRow();
							$CI->html->sDivCol(12);
								$CI->html->sTable(array('class'=>'table paper-table','id'=>'dispatch-items'));
									$CI->html->sTablehead();
										$CI->html->sRow();
											$CI->html->th('Reference');
											$CI->html->th('Item');
											$CI->html->th('Produced Qty');
											$CI->html->th('UOM');
											$CI->html->th('Dispatched Qty');
											$CI->html->th('Dispatch Qty');
											$CI->html->th('');
										$CI->html->eRow();
									$CI->html->eTablehead();
									$CI->html->sTableBody();
										foreach ($items as $ctr => $itm) {
											$CI->html->sRow();
												$CI->html->td($itm->reference);
												$CI->html->td(ucFix($itm->item_name));
												$CI->html->td(num($itm->qty));
												$CI->html->td($itm->uom);
												$CI->html->td(num($itm->dispatched));
												$CI->html->hidden('dispatch_wo_id['.$itm->id.']',$itm->wo_id);
												$CI->html->hidden('dispatch_stg_id['.$itm->id.']',$itm->wo_stg_id);
												$CI->html->hidden('dispatch_item_id['.$itm->id.']',$itm->item_id);
												$usedInput = $CI->html->decimal('','dispatch_qty['.$itm->id.']',null,null,2,array('return'=>true,'style'=>'width:100px;'));
