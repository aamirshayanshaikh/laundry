...
class Inventory_model extends Site_Model{
	...
	public function get_curr_qty_item($item_id,$loc_id){
		$curr_qty = 0;
		$curr_res = $this->site_model->get_tbl('inventory_moves',array('item_id'=>$item_id,'loc_id'=>$loc_id),array('trans_date'=>'DESC','id'=>'DESC'),null,true,'*',null,1);
		if($curr_res){
			$curr_qty = $curr_res[0]->curr_qty;
		}
		return $curr_qty;
	}
	public function move_qty_item($item_id,$loc_id,$qty,$ref_type,$ref,$date,$memo=null){
		$curr_qty = $this->get_curr_qty_item($item_id,$loc_id);
		// echo $curr_qty." - ".$qty;
		// if($qty < 0){
		// 	$curr_qty -= $qty;			
		// }
